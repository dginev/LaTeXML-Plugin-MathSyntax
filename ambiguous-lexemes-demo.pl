...
use Marpa::R2;
use LaTeXML::Demo::MathAST;
use LaTeXML::Global;
use LaTeXML::Common::Error;

package Demo::AmbiguousLexemes;
our $parses = 0;
...
Prefix ::=
  ...
  | TRIGFUNCTION
  | OPFUNCTION
  | FUNCTION action=>first_arg_function
...
Bigop ::=
  BIGOP action=>first_arg_bigop
  | SUMOP action=>first_arg_bigop
  | INTOP
...
sub new {
  my($class,%options)=@_;
    my $grammar = Marpa::R2::Scanless::G->new(
        {   action_object  => 'LaTeXML::Demo::MathAST',
...
sub parse {
  ...
  while (@unparsed) {
    ...
    # 2. Allow for ambiguous lexemes, which have a disjunction of categories
  ...
  my $result = LaTeXML::Demo::MathAST::final_AST(\@values);
...
### DEMO:
my $grammar = new('Demo::AmbiguousLexemes');

my $demo_1_input = "UNKNOWN:x:0 ADDOP:+:1 UNKNOWN:y:2 RELOP:=:3 NUMBER:0:4";
my $demo_1 = $grammar->parse('Anything',\$demo_1_input);

print STDERR "Demo 1 parse: \n",$demo_1->toString(1),"\n\n";


my $demo_2_input = "ID|NUMBER:x:0 ADDOP|POSTFIX:+:1 ID|NUMBER:y:2 RELOP:=:3 NUMBER:0:4";
my $demo_2 = $grammar->parse('Anything',\$demo_2_input);

print STDERR "Demo 2 parse: \n",$demo_2->toString(1),"\n\n";


my $demo_3_input = "SUMOP|FUNCTION:Σ:0 UNKNOWN:x:1 MULOP:*:2 UNKNOWN:y:3";
my $demo_3 = $grammar->parse('Anything',\$demo_3_input);

print STDERR "Demo 3 parse: \n",$demo_3->toString(1),"\n\n";
